defmodule SFU.PeerHandler do
  ...
  def init(_) do
    ...
    SFU.RoomServer.add_peer(:room_server, self())
  ...
  def handle_info({:distribute_video_packet, _track_id, packet}, state) do
    # dbg(
    #   "distributing video packet: #{inspect(state.peer_connection)}, #{inspect(state.out_video_track_id)}, #{inspect(packet)}"
    # )
  ...
  def handle_info({:distribute_audio_packet, _track_id, packet}, state) do
  ...
  defp handle_ws_msg(%{"type" => "offer", "data" => data}, state) do
    ...
    # before we answer, we will loop through every peer and:
    # 1. add a local track to them. We will write this peers packets to those tracks.
    # 2. add a local track to this peer. We will write the other peers outgoing packets onto this one.
  ...
  defp handle_webrtc_msg({:track, track}, state) do
    dbg("received track: #{inspect(track)}")
  ...
  defp handle_webrtc_msg({:rtp, id, packet}, %{in_audio_track_id: id} = state) do
    SFU.RoomServer.receive_audio_packet(:room_server, {self(), id, packet})
    # PeerConnection.send_rtp(state.peer_connection, state.out_audio_track_id, packet)
    {:ok, state}
  end

  defp handle_webrtc_msg({:rtp, id, packet}, %{in_video_track_id: id} = state) do
    SFU.RoomServer.receive_video_packet(:room_server, {self(), id, packet})
    # PeerConnection.send_rtp(state.peer_connection, state.out_video_track_id, packet)
    {:ok, state}
  end
